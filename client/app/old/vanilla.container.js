...
import logger from '~infra/logger.util';
...
class VanillaContainer extends Component {
  ...
  onAddFile = file => {
    axios.post(`${apiUrl}/files`, file).then(resp => {
      this.setState({
  ...
  fetchFiles = () => {
    axios.get(`${apiUrl}/files`).then(resp => {
      this.setState({ files: resp.data });
    });
  };

  render() {
    return <VanillaView actions={this.actions} files={this.state.files} />;
  }
}
