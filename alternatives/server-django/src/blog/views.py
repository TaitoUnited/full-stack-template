...
def posts(request) -> typing.Any:
    if request.method == "GET":
        ...
        if posts_create_serializer.is_valid():
            ...
            return JsonResponse(
                posts_create_serializer.data, status=status.HTTP_201_CREATED
            )
        return JsonResponse(
            posts_create_serializer.errors, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        return JsonResponse(
            {
                "message": "{} Posts were deleted successfully!".format(
                    count[0]
                )
            },
...
def post(request, id) -> typing.Any:
    ...
    if request.method == "GET":
        ...
        post_serializer = PostCreateSerializer(
            post, data=post_data, partial=True
        )
