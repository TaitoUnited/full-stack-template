...
declare module "koa-joi-router" {
  ...
  namespace createRouter {
    ...
    type FullHandler = (ctx: Koa.ParameterizedContext, next: () => Promise<any>) => any;
    interface NestedHandler extends
      ReadonlyArray<PartialHandler | FullHandler | NestedHandler> { }
    ...
    type Method = {
      ...
      (path: string | RegExp, middleware: Koa.Middleware, handler: Handler): Router;
      (path: string | RegExp, config: Spec, handler: Handler): Router;
      (path: string | RegExp, config: Spec, middleware: Koa.Middleware,
