...
export default class FileRoute extends BaseRoute {
  ...
  routes() {
    ...
    this.router.get('/', async (ctx, next) => {
      ctx.body = await this.fileService.fetch(ctx.appCtx, ctx.params);
      next();
    });
    ...
    this.router.put('/:id', async (ctx, next) => {
      ctx.body = await this.fileService.update(ctx.appCtx, ctx.request.fields);
      next();
    });

    // Patch a file (partial update)
    this.router.patch('/:id', async (ctx, next) => {
      ctx.body = await this.fileService.patch(ctx.appCtx, ctx.request.fields);
      next();
    });
